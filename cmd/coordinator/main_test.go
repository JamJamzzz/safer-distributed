...
// configuration must never stop the coordinator from starting. It sets
...
func TestSetupTelemetry_FailsOpenOnMalformedConfig(t *testing.T) {
	...
	shutdown := setupTelemetry(context.Background(), telemetry.Config{ServiceName: "safer-coordinator"})
	if shutdown == nil {
